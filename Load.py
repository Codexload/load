...
def passwrd():
	...
	print(f'[{h}AKASH{u}]{h} OK : {h}%s '%(ok))
	print(f'{k}[{k}AKASH{h}]{k} CP : {k}%s{u} '%(cp))
	print('')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}AKASH>{P}[{k}{loop}{P}/{h}{len(id)}{P}]OK{P}[{H}{ok}{P}]CK{P}[{k}{cp}{x}]KST[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
